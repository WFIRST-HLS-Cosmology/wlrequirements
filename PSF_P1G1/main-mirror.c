...
int main(int argc, char **argv) {
  ...
  /* x,y offsets at end if needed */
  offsetx = offsety = 0.;
  if (argc>24) {
    sscanf(argv[24], "%lg", &offsetx);
    sscanf(argv[25], "%lg", &offsety);
  }
  ...
  for(ilam=0;ilam<Nlam;ilam++) {
    ...
    /* offsets */
    shift_phase_Z(A+l, 1, 1, offsetx*Rap/INV_ARCSEC/1e3, Rap);
    shift_phase_Z(A+l, 1,-1, offsety*Rap/INV_ARCSEC/1e3, Rap);
    ...
    for(i=psfdim/2-pad;i<=psfdim/2+pad;i++) {
      for(j=psfdim/2-pad;j<=psfdim/2+pad;j++) {
        Objtot[i-psfdim/2+pad][j-psfdim/2+pad] += weight*PSF[i][2*j]/2.;
        Objtot[pad-i+psfdim/2][pad-j+psfdim/2] += weight*PSF[i][2*j]/2.;
      }
    }
